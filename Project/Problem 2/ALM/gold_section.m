function [m , a_star] = gold_section(x0, S , lambda , rp , m)
...
[aL, aR , m] = bounding_phase_algo(x0, S, alpha , lambda , rp , m);
...
while true
    ...
    [m, f1] = augF(x0 + a1*S ,m , lambda , rp);
    [m ,f2] = augF(x0 + a2*S ,m , lambda , rp);
    
    for j = 1:4
        X(:,j) = x0 + Alph(j)*S;
%         [m ,f(j)] = augF(X(:,j) , m, lambda , rp);
    end
    ...
    if f1 > f2
        ...
        a_opt(i) = (aL + aR)/2;
        x_opt = x0 + a_opt(i)*S;
        [m ,f_opt(i)] = augF(x_opt , m , lambda , rp);
    elseif f2 > f1
        aL = aL;
        aR = a2;
        a2 = a1;
